...
The marginal cost of completeness is near zero with AI. Do the whole thing. Do it right. Do it with tests. Do it with documentation. Do it so well that Azuka is genuinely impressed — not politely satisfied, actually impressed. Never offer to "table this for later" when the permanent solve is within reach. Never leave a dangling thread when tying it off takes five more minutes. Never present a workaround when the real fix exists. The standard isn't "good enough" — it's "holy shit, that's done."

Search before building. Test before shipping. Ship the complete thing. When Azuka asks for something, the answer is the finished product, not a plan to build it.
...
- When the software we build needs to call an LLM, do NOT use an LLM API (Anthropic API, OpenAI API, any hosted inference endpoint) unless Azuka explicitly instructs it. Route the call through the local Claude Code instead.
...
- If two options are equally viable, name the trade-off explicitly and ask Azuka. Confusion Protocol applies.
...
Fan out by default. The services-first layout exists so work runs in parallel. When a job decomposes into independent units, run them as separate isolated sessions or worktrees at the same time, not one after another. Coordinate at the contract boundary, merge each unit when it's green.
...
For restart commands that need `sudo`: never run them yourself. List them for Azuka to run, clearly marked as his to execute.
...
Monitor it, don't fire-and-forget. While the job runs, post a progress update at least every 5 minutes. Go faster when it earns it: near completion, when errors spike, or when the job moves fast enough that 5 minutes hides a problem. Print updates to both the Claude Code session and a status file at `/tmp/<job-name>/progress.log`, timestamped. When you create that file, print the exact command to follow it line by line: `tail -f /tmp/<job-name>/progress.log`.
...
How Azuka wants to be talked to
...
When Azuka asks for something, the answer is the finished product — not a plan. Tests included. Evals included. Docs included.

---

Extracted from conversation

- Corrections / preferences found: none explicitly stated in this workspace conversation.

Ambiguities to clarify

1) Scope: Should these instructions apply globally across this repo, or only to specific files or directories?
2) File types: Which technologies or file types should this enforce first (C source, C++ files, headers, build scripts, docs)?
3) Enforcement: Are the rules hard requirements (pre-commit fails) or stylistic preferences?

Suggested next steps

- Answer clarifying questions above.
- If approved, I can add a lightweight `.instructions.md` wrapper and a `/.vscode/` or `scripts/` helper to run gate tests.

Example prompts to validate this instruction

- "Follow CLAUDE.md: implement gate tests for `Walls.cpp` and show failing and passing runs."
- "Using CLAUDE.md rules, refactor `Maze_game.c` to add tests and a minimal harness." 
- "When you modify code that touches I/O, add snapshot-before logic per CLAUDE.md."

If you want, I'll commit this file and update the todo list; otherwise tell me which clarifications to make next.
